...
export default function App(){
 ...
 return <div className="app-shell">
  ...
  <main className="main-content"><div className="breadcrumb"><span>VTB Business</span><ChevronRight size={13}/><span>Продукты и услуги</span><ChevronRight size={13}/><strong>Партнёрские сервисы</strong></div>
   ...
   <section className="contact-strip"><div className="contact-description"><span className="contact-symbol"><Headphones size={25}/></span><div><h2>Быстрая связь с Банком</h2><p>Поможем с вопросами по VTB Business</p></div></div><div className="contact-actions">{contacts.map(c=>{const content=<>{c.id==='telegram'?<Send size={19}/>:<MessageCircle size={20}/>}<span>{c.name}{!c.url&&<small>Контакт уточняется</small>}</span>{c.url&&<ArrowUpRight size={15}/>}</>;return c.url?<a href={c.url} key={c.id} target="_blank" rel="noopener noreferrer">{content}</a>:<button key={c.id} disabled>{content}</button>})}</div></section>
